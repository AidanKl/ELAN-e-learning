...
toon_balk: true
intro: Je maakt de dataset begrijpelijk en analyseklaar. Dit is de fase waarin de meeste stille fouten
...
- subtitel: Kritieke bijzonderheden in de data
  ...
  - type: valkuil
    ...
    inhoud: Een HIS wordt bijgehouden voor de zorg, niet voor onderzoek. Een lege registratie betekent
      niet dat een aandoening afwezig is.
  - type: letop
    kop: Transitie van diagnose
    inhoud: Een diagnose kan worden bijgesteld; dat gebeurt niet altijd. JRN bevat daarom twee ICPC-kolommen.
  - type: letop
    kop: Verschillen tussen HIS-systemen en omgevingen
    inhoud: Variabelen zijn net iets anders gevuld per HIS. INS/PAT gescheiden op I-schijf, samengevoegd
...
- subtitel: Change tracking & ontdubbelen
  blokken:
  - type: tekst
    inhoud: STIZON haalt data op in 'snapshots' en voegt `StartDate`/`EndDate` toe. Hierdoor staan er
      mogelijk dubbele records in tabellen als EPS en COR.
  - type: valkuil
    kop: Ontdubbelen is bijna altijd nodig
    inhoud: Verwijder records met een `EndDate` en ontdubbel op de sleutelvariabelen. Zie het Scriptoverzicht.
  - type: todo
    inhoud: 'NOG-AAN-TE-VULLEN: centrale lijst met bekende datakwaliteitsproblemen per databron en standaard
      cleaning-workflows.'
naslag:
